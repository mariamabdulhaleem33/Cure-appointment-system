        ...
        <div className="flex flex-col items-center gap-4 w-full">
          {questions.map((q) => (
            <div key={q.id} className="flex flex-col w-full max-w-[800px]">
              <div
                className="w-full h-[59px] flex justify-between items-center p-4 rounded-[8px] bg-white shadow-lg hover:shadow-xl transition-shadow duration-300 cursor-pointer"
              ...
              {openId === q.id && (
                <div className="w-full bg-white px-4 pb-4 text-gray-600 border-t border-gray-200">
                  {q.answer}
                </div>
              )}
            </div>
